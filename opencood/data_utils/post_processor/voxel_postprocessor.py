...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def post_process(self, data_dict, output_dict):
        ...
        if 'ego' not in data_dict and 'ego' not in output_dict:
            data_dict = {'ego': data_dict}
            output_dict = {'ego': output_dict}
        
        for key in data_dict['ego']:
            if isinstance(data_dict['ego'][key], list):
                assert len(data_dict['ego'][key]) == 1, 'During validation/testing, ' \
                                                    'the batch size should be 1.'
                data_dict['ego'][key] = data_dict['ego'][key][0]
        ...
        return dict(
            pred_box3d_tensor=pred_box3d_tensor,
            scores=scores
        )

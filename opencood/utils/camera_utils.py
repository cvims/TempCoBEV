...
def load_rgb_from_files(camera_list):
    ...
    for (i, camera_file) in enumerate(camera_list):
        ...
        camera_dict['camera%d' % i] = np.array(PIL.Image.open(camera_file))[..., ::-1]
...
def project_3d_to_camera(objects, intrinsic, extrinsic):
    ...
    for i in range(objects.shape[0]):
        ...
        if extrinsic.shape == (3,3):
            extrinsic = np.r_[extrinsic, np.array([[0, 0, 0]])]
            extrinsic = np.c_[extrinsic, np.array([[0], [0], [0], [1]])]
...
def filter_bbx_out_scope(objects, image_w, image_h):
    ...
    Returns
    -------
    Remaining bounding boxes and the mask indicating which bounding boxes
    are filtered out.
    """
    ...
    has_negative_z = np.all(objects[:, :, 2] > 0.0, axis=1)
    points_in_canvas_mask = points_in_canvas_mask & has_negative_z   
...
def draw_2d_bbx(image, objects, color=(255, 0, 0), thickness=2):
    ...
    object_2d_coords = np.zeros((objects.shape[0], 2, 2))
    filtered_objects, _ = filter_bbx_out_scope(objects, image_w, image_h)

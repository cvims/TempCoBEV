...
class PointPillarLoss(BaseLoss):
    ...
    def _forward(self, output_dict, target_dict):
        """
        Parameters
        ----------
        output_dict : dict
        ...
        psm = output_dict['psm']
        # new
        target_dict = target_dict['label_dict']
    ...
    def logging(self, epoch, batch_id, batch_len, logger: BaseLogger, pbar: tqdm = None) -> None:
        ...
        if pbar is None:
            print("[epoch %d][%d/%d], || Loss: %.4f || Reg Loss: %.4f"
                " || Conf Loss: %.4f" % (
                    epoch, batch_id + 1, batch_len,
                    total_loss.item(), reg_loss.item(), conf_loss.item()))
        else:
            pbar.set_description("[epoch %d][%d/%d], || Loss: %.4f || Reg Loss: %.4f"
                  " || Conf Loss: %.4f" % (
                      epoch, batch_id + 1, batch_len,
                      total_loss.item(), reg_loss.item(), conf_loss.item()))
